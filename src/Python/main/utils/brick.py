...
try:
    ...
    from Python.main.project.utils.dummy import Enumeration, FirmwareVersionError, SensorError, BrickPi3
        ...
        if self.SensorType[port_index] == self.SENSOR_TYPE.CUSTOM:
            ...
            if (reply[3] == 0xA5):
                if ((reply[4] == self.SensorType[port_index] or (self.SensorType[port_index] == self.SENSOR_TYPE.TOUCH
                                                                 and (reply[4] == self.SENSOR_TYPE.NXT_TOUCH or reply[
                            4] == self.SENSOR_TYPE.EV3_TOUCH)))):
                    return reply[5]
                else:
                    return SENSOR_STATE.INCORRECT_SENSOR_PORT
            else:
